...
public class Dribble : PlayerStepAction {
    ...
    public override bool IsStarted(Player player)
    {
        ...
        {
            isStarted = Vector3.Distance(player.transform.position, start) < minCompletionDistance && player.hasBall;
            if (isStarted)
            {
    ...
    public override bool IsDone(Player player)
    {
        ...
        else if (ValidateAction(player) && Vector3.Distance(player.transform.position, end) < minCompletionDistance && player.hasBall)
        {
            isStarted = false;
            isDone = true;
